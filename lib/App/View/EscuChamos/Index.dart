...
int postId_ = 0 ;
bool? likeState;
class IndexEscuChamos extends StatefulWidget {
  final int? userId;
  IndexEscuChamos({this.userId});
  @override
  _IndexEscuChamosState createState() => _IndexEscuChamosState();
}

class _IndexEscuChamosState extends State<IndexEscuChamos> {
  List<Datum> posts = [];
      ...
      if (mounted) {
        if (response is PostModel) {
          if (postId_ != 0) {
            ...
            posts.removeWhere((post) => post.id == postId_);
          });
        //   await showDialog(
        //     context: context,
