...
class ReportCommandUpdate {
  final ReportUpdate _reportService;

  ReportCommandUpdate(this._reportService);

  Future<dynamic> execute(
    String model, int objectId, int statusId,
  ) async {
    try {
      var response =
          await _reportService.updateReport(model, objectId, statusId);
